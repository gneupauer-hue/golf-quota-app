...
export type PayoutPredictionsSummary = {
  ...
  individualRankings: SideGameResults["individualRankings"];
...
export function formatPayoutAuditStatus(label: string, difference: number) {
  if (roundCurrency(difference) === 0) {
    return "OK";
  }

  if (difference < 0) {
    const missingLabel = label.includes("Indy Paid") ? "Unassigned" : "Missing";
    return `${missingLabel} $${Math.abs(roundCurrency(difference))}`;
  }

  return `Overpaid by $${roundCurrency(difference)}`;
}
...
export function calculateTeamStandings(rows: CalculatedRoundRow[]): TeamStanding[] {
  ...
  for (const current of grouped.values()) {
    const { frontQuota, backQuota } = splitQuota(current.totalQuota);
    current.frontQuota = frontQuota;
    current.backQuota = backQuota;
...
function getIndividualPotTotal(playerCount: number) {
  return roundCurrency(
    getIndividualPayoutTable(playerCount).reduce((sum, amount) => sum + amount, 0)
  );
}
...
export function calculateGoodSkins(rows: CalculatedRoundRow[]): SkinsResult {
  ...
  const valuePerSkin = totalSkinSharesWon > 0 ? Math.floor(totalPot / totalSkinSharesWon) : 0;
  ...
  holes.forEach((hole) => {
    ...
    const holePayout = roundCurrency(valuePerSkin);
  ...
  const totalDistributed = roundCurrency(
    winners.reduce((total, winner) => total + winner.payout, 0)
  );
  const leftover = roundCurrency(Math.max(0, totalPot - totalDistributed));
...
export function calculateSideGameResults(rows: CalculatedRoundRow[]): SideGameResults {
  ...
  const skinsPot = rows.length * 10;
  const totalPot = roundCurrency(teamPot + indyPot + skinsPot);
...
export function calculatePayoutPredictions(
  ...
  return {
    ...
    individualRankings: sideGames.individualRankings,
    frontProjectedTotal,
    backProjectedTotal,
    totalProjectedTotal,
    indyProjectedTotal,
...
export function calculatePayoutAudit(
  rows: CalculatedRoundRow[],
  mode: RoundMode
): PayoutAudit {
  const sideGames = calculateSideGameResults(rows);
  ...
  const totalMatchPaid = roundCurrency(payoutSummary.players.reduce((sum, player) => sum + player.total, 0));
